...
def train(train_loader, test_loader, tnet, criterion, optimizer, epoch):
    ...
    for batch_idx, (data, label) in enumerate(train_loader):
        ...
        if args.mixup:
            ...
            for j in range(len(outputs)):
                loss += loss_func(criterion, outputs[j])
            '''
        else:
            outputs = tnet(data)
            ...
            '''
            for j in range(len(outputs)):
                loss += criterion(outputs[j], label)
            '''
    ...
    for batch_idx, (data, label) in enumerate(test_loader):
        ...
        pred = F.softmax(args.a * classification[0] + args.b * classification[1] + args.c * classification[2])
        #pred = F.softmax(classification[3])
        tart = label
    ...
    ece = ece_score(pre, tar, n_bins=args.bins)
    
    print('ECE of testset: %.3f %%' % (100 * ece))
