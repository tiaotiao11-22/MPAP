...
_default_anchors_setting = (
    ...
    #dict(layer='p4', stride=2, size=2, scale=[2 ** (1. / 3.), 2 ** (2. / 3.)], aspect_ratio=[0.667, 1, 1.5]),
    #dict(layer='p5', stride=2, size=3, scale=[1, 2 ** (1. / 3.), 2 ** (2. / 3.)], aspect_ratio=[0.667, 1, 1.5]),
)
...
class ATM(torch.nn.Module):    
    ...
    def forward(self, feature_tensor, feature_vector):     # [1024]  [1024]
        ...
        #feature_vector = feature_vector.view([b, self.inp_vec, w, h])
...
class ResNet(nn.Module):
    ...
    def forward_once(self, x):
        ...
        '''
        # MAF v2
        w1 = torch.sum((embedding_b2*embedding_b3),1)
        w2 = torch.sum((embedding_b1*embedding_b3),1)
        w3 = torch.sum((embedding_b1*embedding_b2),1)

        w1_n = w1/(w1+w2+w3)
        w2_n = w2/(w1+w2+w3)
        w3_n = w3/(w1+w2+w3)
        w1_n = w1_n.unsqueeze(1)
        w2_n = w2_n.unsqueeze(1)
        w3_n = w3_n.unsqueeze(1)

        classification = w1_n * classification_b1 + w2_n * classification_b2 + w3_n * classification_b3
        '''
